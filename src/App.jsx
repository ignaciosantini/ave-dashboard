...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const raw = localStorage.getItem("ave_data");
        ...
        if(r?.value){
          ...
          localStorage.setItem("ave_data",JSON.stringify({stock:SEED_STOCK,months:SEED_MONTHS}));
        }
      }catch{setStock(SEED_STOCK);setMonths(SEED_MONTHS);}
    })();
  },[]);

  const persist=useCallback(async(s,m)=>{setStock(s);setMonths(m);try{localStorage.setItem("ave_data",JSON.stringify({stock:s,months:m}));}catch{}},[]);
